      ...
      <div className="flex h-full min-h-0 min-w-0 flex-col gap-3 overflow-x-hidden overflow-hidden">
        <div className="shrink-0 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between min-w-0">
          <div className="min-w-0">
            ...
            <h1 className="text-lg sm:text-xl font-semibold tracking-tight text-slate-900">Campaigns</h1>
            <p className="text-xs text-slate-500">{campaigns.length} total</p>
          </div>
          {canManage && (
            <Button asChild size="sm" className="h-8 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-xs w-full sm:w-auto">
                ...
                <TableBody>
                  {campaigns.map((c) => (
                    <TableRow key={c.id} className="hover:bg-slate-50/60">
                      <TableCell className="px-3 py-2.5 text-sm font-medium text-slate-900 max-w-[140px] sm:max-w-[220px] truncate">{c.name}</TableCell>
                      <TableCell className="px-3 py-2.5 text-xs text-slate-600 capitalize whitespace-nowrap">{c.status.replace("_", " ")}</TableCell>
                      <TableCell className="px-3 py-2.5 text-xs tabular-nums text-slate-700 whitespace-nowrap">₦{c.spent_amount.toLocaleString()}</TableCell>
                      <TableCell className="px-3 py-2.5 text-xs text-slate-600 max-w-[120px] truncate">{formatGeoTargets(c.target_geo)}</TableCell>
                      <TableCell className="px-3 py-2.5 text-xs text-slate-600 max-w-[120px] truncate">{c.placements.join(", ")}</TableCell>
